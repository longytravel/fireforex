...
| MT5 M1 downloader | `ff/data/mt5_m1_downloader.py` | Mirror of bi5 downloader for MT5 broker terminal — same parquet shape into `BackTestData_MT5/` | ⚠️ | Pair-coverage gap: only pairs the laptop's MT5 market-watch includes get pulled (memory `project_mt5_replay_pair_coverage.md`). Windows-only (MetaQuotes binary). M1 `spread` field is bar-close-tick only and floor-biased (issue #39); cost-realism prefers the tick downloader below. |
| MT5 tick downloader | `ff/data/mt5_tick_downloader.py` | Per-pair tick history (real bid/ask per quote change) via `mt5.copy_ticks_range()` → `{pair}_TICK.parquet` in `BackTestData_MT5/`. Append-only, daily chunking, resumable. | ✅ | Windows-only. Reuses `mt5_m1_downloader._ensure_mt5_connected/_symbol_for/_write_atomic`. The trustworthy spread source for cost-realism (PR #42) — no floor-bias because every tick is a genuine quote. |
...
| MT5 broker bridge | `ff/live/broker_mt5.py` | Thin wrapper over `MetaTrader5` pip package; documented failure taxonomy (REQUOTE / REJECT / partial fill / disconnect / clock drift / duplicate plan); zero MQL5 EA. Exposes `current_spread_pips(pair)` for the runner's submit-time guard | ✅ | No silent retries; every outcome logged. Windows-only. Rejected orders no longer create phantom positions (memory `MT5 Rejected Orders No Longer Create Phantom Positions`). |
...
| Live trade reconciler | `ff/live/reconcile.py` | Join per-trade BT log against MT5 deal history; classify match / missing / extra / mismatched-{entry,exit,pnl}; key `(pair, direction, signal_bar_ts)` with sweep window for clock drift. `MatchedRow` carries cost-realism columns (raw / overlay / adjusted / gated / effective) so the headline report shows the same decomposition as the History tab | ✅ | No MT5 dependency — takes pre-fetched data so unit tests can mock both sides. Stage-6 reconcile script calls this. |
...
| Cost table builder | `ff/cost_realism/cost_table.py` | Build `artifacts/cost_table.json` from MT5 tick (preferred) or M1 (legacy fallback) parquets — per-pair × per-session **mean** spread (pips), flat per-side commission, default-then-telemetry-fed slippage. Validator raises on absurd-upper (>50 pips) and per-pair tight-floor (USD-majors <0.05 / crosses <0.15) violations; orchestrator catches per-pair errors and continues. Each entry tagged `spread_source: "tick" \| "m1"`. Also exposes `per_trade_overlay_charge_pips(pair, path)` — aggregate liquid-session bt-vs-ic delta consumed by the optimiser's pick-best path. | ✅ | Reads `MT5_DATA_ROOT` parquets; commission is flat 0.35 pips/side until per-pair statement evidence justifies overrides. PR #38 switched median→mean; PR #42 switched the source from M1 bar-close to tick bid/ask, sidestepping the floor-bias from issue #39. |
...
| MT5 tick bulk fetcher | `scripts/fetch_mt5_ticks.py` | Bulk download MT5 tick history for the 28 default pairs via `ff.data.mt5_tick_downloader.download`. Append-only, per-pair errors don't abort the batch. | ✅ | Windows-only (needs running MT5 terminal). Default 90-day window; `--days N` to widen. |
...
| `tests/cost_realism/test_cost_table.py` | Cost-table generator — per-session mean spreads, missing-pair skip, commission lookup, per-pair lower-bound floor (validator raises + builder skips per-pair), TZ-aware UTC bucketing, floor-biased distribution regression, tick-parquet preferred over M1 fallback (PR #42), **`per_trade_overlay_charge_pips` helper for the pick-best overlay (Option C)**. | ✅ | 17 tests; uses synthetic parquet fixtures + JSON cost-table fixtures, hermetic. |
| `tests/test_pick_best_adjusted_proxy.py` | `pick_best`'s `objective_array` override path — synthetic per-trial array ranks higher-is-better, tie-breakers fall back to `metrics_out`, profitability filter still applies, length-mismatch raises. | ✅ | 6 tests; pure unit, hermetic. |
| `tests/test_mt5_tick_downloader.py` | MT5 tick downloader merge dedupe (preserve distinct same-ms ticks) and broker UTC offset stale-tick guard. | ✅ | 5 tests; pure unit (no MT5 needed), hermetic. |
