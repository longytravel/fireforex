...
| HTTP endpoints | `app/routes.py` | All `/api/*` endpoints — defaults, run, jobs, baseline, instances, EA catalog, docs proxy | ✅ | Issue #12 fixed: every `/runs/...` endpoint now goes through `_resolve_run_npz` which validates the regex AND asserts the resolved path lives under `artifacts/runs` (defends against symlink escape). `/runs/{run_id}/trades.csv` was the last weak link; switched to the shared helper. |
