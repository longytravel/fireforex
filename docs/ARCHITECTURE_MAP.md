...
| Cost table builder | `ff/cost_realism/cost_table.py` | Build `artifacts/cost_table.json` from MT5 tick (preferred) or M1 (legacy fallback) parquets — per-pair × per-session **mean** spread (pips), flat per-side commission, default-then-telemetry-fed slippage. Validator raises on absurd-upper (>50 pips) and per-pair tight-floor (USD-majors <0.05 / crosses <0.15) violations; orchestrator catches per-pair errors and continues. Each entry tagged `spread_source: "tick" \| "m1"`. | ✅ | Reads `MT5_DATA_ROOT` parquets; commission is flat 0.35 pips/side until per-pair statement evidence justifies overrides. PR #38 switched median→mean; PR #42 switched the source from M1 bar-close to tick bid/ask, sidestepping the floor-bias from issue #39. |
