...
- [x] **Live reconcile + execution guards bundle** (shipped 2026-04-26 night, branch `fix/live-reconcile-and-guards`) — three cost-realism follow-ups in one PR.
  - **#32**: `ff/live/reconcile.py` `MatchedRow` now carries `bt_raw_pnl_pips`, `bt_overlay_delta_pips`, `bt_adjusted_pnl_pips`, `bt_gated_out_reason`, `bt_effective_pnl_pips`. `ReconcileReport.by_pair` adds `matched_effective_pnl_pips_bt` so a gated trade reads as 0 in the rollup. Legacy backtests fall back to `bt_pnl_pips` so the new columns always have a number.
  - **#33**: `MT5Broker.current_spread_pips()` queries a live `mt5.symbol_info_tick`; runner uses it in `_evaluate_and_fire` immediately before the execution guard. Stops a stale closed-bar mean from letting a spike-time fire through the 3-pip cap. Falls back to closed-bar value when the broker shim does not expose the new method (synthetic tests).
  - **#34**: post-fill slippage cap. After `submit_market_order` returns the runner computes signed `fill_slippage_pips` and feeds it to `gate_rules.is_slippage_too_wide`. When the cap fires, the position is closed via `broker.close_position(reason="slippage_3p")`, never registered in `state.open_positions`, and the ticket row is tagged `slippage_killed=True`.
  - 10 new tests across `test_reconcile.py`, `test_broker_mt5_submit.py`, `test_live_runner_synthetic.py`. Full suite: 270 passed.
