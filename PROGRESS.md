...
- [x] **Mega brute-force sweep foundation** (shipped 2026-04-26, PR #40) — signal libraries are stored as contiguous per-variant slices so Rust can evaluate only the selected variant; lean chunked sweep mode streams compact metrics sidecars for >50k trial runs; Run page scatter/trial APIs read lean artifacts; web/API cap raised to 50M with rich artifacts still capped at 50k.
